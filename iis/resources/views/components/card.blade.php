@php($user = Auth::user())
<div class="bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700" onclick="redirectToEventDetail('{{ route('event_detail', ['event' => $event]) }}');">
    <a href="#">
        <img class="p-8 rounded-t-lg" src="{{ $event->logo ? $event->logo : '/images/logotickify.png' }}" alt="product image" />
    </a>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex flex-col items-end">
                @if($user && $event->host_id == $user->id)
                    <button
                        x-data="{ event: @json($event) }"
                        x-on:click="$dispatch('edit-event', event)"
